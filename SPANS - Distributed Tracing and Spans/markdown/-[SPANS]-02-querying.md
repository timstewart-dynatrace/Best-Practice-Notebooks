...
> **Following on (SaaS 1.347 — staged rollout from 09/08/2026): those derived routes now participate in matching.** Calculated `http.route` values are included in URL path pattern-matching conditions, so a request-naming or service-detection rule written against a URL path can now match on a route that OneAgent never sent. Combined with the 1.346 derivation above, a rule that matched nothing while endpoints collapsed to `/*` can start matching without anyone editing it — review URL path pattern conditions for intended behaviour rather than assuming they are inert.
