...
| Lambda and serverless: cold starts, throttles, FaaS invocation metrics (`dt.service.faas_invoke.count`), Service Detection v2 (GA), API Gateway, Step Functions, DynamoDB, end-to-end serverless tracing | `-[CLOUD]-04-aws-lambda-serverless.md` |
| Connecting Azure: Clouds-app Azure connection vs Azure Native Dynatrace Service, Entra ID app registration with a federated identity credential and Monitoring Reader, Smartscape `AZURE_MICROSOFT_*` queries, forwarding activity/Entra ID/resource logs via diagnostic settings and regional Event Hubs, AKS (Operator install, Windows/virtual-node limits, Deployment Safeguards, kubenet retirement, control-plane logs), subscription/resource-group governance | `-[CLOUD]-05-azure-integration.md` |
