...
Per-domain depth lives in the canonical series (linked at each step). For recommended defaults, the decision matrix, validation DQL, anti-patterns, and the cross-series next-steps map, see **ONBRD-99: Best Practice Summary**.
...
| Account-level decisions in place | tenant region, SaaS vs Managed, account hierarchy. If not yet decided, see **NR2DT-00: Tenant Prerequisites — Minimum Setup Before Migration** — the same checklist generalizes beyond New Relic migrations. |
...
- **Deep dive:** **ONBRD-99: Best Practice Summary** § 3 Validation Queries for the canonical set; SPANS, DBMON, WEBRUM, BIZEV for per-domain DQL patterns.
...
Verify before moving from one phase to the next. The DQL queries in **ONBRD-99: Best Practice Summary** § 3 Validation Queries make these gates concrete.
