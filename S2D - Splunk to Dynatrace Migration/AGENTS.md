...
| Scheduled workflow-based alerts when detectors don't fit: workflow structure, event-creation JavaScript, cron schedules, drawbacks and license cost, and Davis event attribution — you set `entitySelector` and Davis populates `dt.smartscape_source.id`; an unattributed event falls back to the environment entity, so a scheduled loop emitting one event per breaching row collapses them all into a single permanently-open problem. Skip rows you cannot attribute rather than sending them | `-[S2D]-05-workflow-alerts.md` |
