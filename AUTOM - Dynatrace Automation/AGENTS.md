...
| Terraform provider — API Token vs Platform Token vs OAuth client auth, `DYNATRACE_HTTP_OAUTH_PREFERENCE`, resource catalog by domain, `-export -ref -id`, state leakage, mint-out-of-band; Gen3 platform resources (workflows, documents, segments, maintenance) and the two categories that reject a Platform Token whatever its scopes and need OAuth client credentials — Grail buckets and IAM | `-[AUTOM]-04-terraform.md` |
