...
| Terraform against Gen3 platform resources (workflows, documents, segments, maintenance) — and the two categories that reject a Platform Token whatever its scopes and need OAuth client credentials: Grail buckets and IAM | `-[AUTOM]-04-terraform.md` |
...
| `-export -migrate` vs `-ref` (mutually exclusive; `-migrate` is the tenant-to-tenant one), bulk vs iterative export, the export output structure, importing to the target | `-[AUTOM]-08-migration-automation.md` |
