...
| CI/CD pipelines — GitHub Actions, GitLab, Bitbucket, Bamboo, Azure DevOps, ArgoCD/Flux, Operator GitOps, Vault, drift detection, ephemeral Platform Token minting; policy-as-code gates (OPA/Conftest, Sentinel) including the `blocked_at_upgrade.rego` gate that fails the build when a resource type whose schema is removed at upgrade enters the repo, run warn-then-deny as a migration ratchet | `-[AUTOM]-07-cicd-integration.md` |
