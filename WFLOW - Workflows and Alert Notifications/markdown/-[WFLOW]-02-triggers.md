...
> **Series:** WFLOW — Workflows and Alert Notifications | **Notebook:** 2 of 10 | **Created:** January 2026 | **Last Updated:** 07/31/2026
...
**Where the entity fields come from.** `affected_entity_ids`, `root_cause_entity_id`, and `impacted_entities` are populated from the Davis events Dynatrace grouped into this problem, and that grouping runs on `dt.smartscape_source.id` — the Smartscape entity ID each event carries. Events naming the same entity within the correlation window merge into the single problem this payload describes.

That has a direct consequence for anyone reading these fields in a workflow: a problem assembled from **unattributed** events arrives with those fields thin or empty, so routing that branches on `affected_entity_types`, `entity_tags`, or `root_cause_entity_id` silently falls through to its default. If a workflow keeps landing on the fallback branch, suspect the upstream detector's event template before rewriting the routing logic — an event template that leaves `dt.smartscape_source.id` unset also opens a separate problem on every firing, so the symptom usually arrives alongside a problem-count spike. AIOPS-03 §1 covers the correlation rules, AIOPS-02 §8 finds unattributed detectors in your own tenant, and ALERT-99 §3 explains which Davis data object to count when you audit.
