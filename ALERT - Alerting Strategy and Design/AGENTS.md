...
| Principles on one page, the complete end-to-end setup checklist, the ongoing audit loop — the 0.1%-of-observed-time noise yardstick (~1–1.5 min/day, ~43 min/month), true-positive / false-positive / under-review classification, and two standing trend queries (denoising ratio, correlation coverage) that carry no published health bands, so trend them against your own baseline — and the cross-series build map, plus two standing trend queries built from the events-and-problems pair: denoising ratio (problem-worthy events ÷ problems, how much Causal AI grouping is actually consolidating) and correlation coverage (the share of events carrying `dt.smartscape_source.id`) — a falling coverage trend is a config problem in the event templates, not a Davis problem | `-[ALERT]-99-best-practice-summary.md` |
