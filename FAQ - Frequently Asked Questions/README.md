...
19. [How Do I Bring a Third-Party SaaS Platform's Telemetry Into Dynatrace?](markdown/-[FAQ]-19-third-party-saas-telemetry-integration.md) — The reusable pattern for a platform you do not run: mapping questions to signal classes, letting the vendor's transport pick the ingestion route, extracting signal at ingest and discarding raw records (and the stage-order trap that silently defeats it), modeling vendor objects as `CUSTOM_*` Smartscape topology, and isolating whatever you do retain
20. [How Do I Monitor Zscaler With Dynatrace?](markdown/-[FAQ]-20-monitoring-zscaler.md) — The worked example of FAQ-19: why ZIA and ZPA logs cannot replace ZDX, the transport split across the three product lines that breaks a single-route design, direct-vs-multi-home ingestion architectures, what to extract and what to discard, a five-node topology model, field mapping and metric naming, and when synthetic monitoring adds anything over ZDX
