...
| Getting the right alerts to the right people: the four axes an alerting design conflates — what **fires** (anomaly detector, scoped by segment), who gets **paged** (problem-triggered workflow trigger), who may **see** the problem (the IAM access model), what a user **filters** in the UI (segments); the documented problem-trigger option list (problem state, event category, severity, affected-entity tags, Delay, Updates, additional custom filter query) and the absence of any built-in team or routing field; routing on entity tags + severity + a custom attribute you set, matched with a DQL matcher; Davis correlation as the reason to trigger on problems rather than individual alerts; the **Delay** option for duration suppression (`dt.duration_marker`) and its unresolved conflict with the upgrade guide; and three anti-patterns — segments-scope-notification, routing-replaces-visibility, route-before-enrich | `-[FAQ]-21-alert-notification-routing.md` |
