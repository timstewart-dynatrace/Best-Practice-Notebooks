...
| Building/choosing anomaly detectors — static threshold vs auto-adaptive vs seasonal baseline vs multi-dimensional vs novelty/forecast, records-based detectors for sparse signals, metric events, Settings 2.0 schemas, the workflow problem-trigger delay (SaaS 1.344) as a notification-layer setting separate from the detector — it does not change when Davis opens a problem, testing with Davis analyzers; also finding and quieting noisy detectors — which fire most, and measuring what a shadow-deployed detector would have done before enabling it; also how much history a baseline needs before it's trustworthy (7-day auto-adaptive / 14-day seasonal reference windows), and a pre-publish checklist for custom detectors (cardinality, attribution, cost) | `-[AIOPS]-02-anomaly-detection.md` |
| Davis problems and root cause — how Causal AI groups signals via Smartscape, the correlation keys (`dt.smartscape_source.id`) and the silent over-merge failure when a custom alert leaves it unset — the event falls back to the environment entity rather than going unattributed, so unrelated alerts collapse into one problem with no usable root cause, `dt.davis.problems` vs `dt.davis.events` DQL, active problem feed, MTTR reporting, severity rollups; also when an empty root cause is expected Davis behavior rather than a detection gap, and how to trend RCA attachment rate over time | `-[AIOPS]-03-davis-ai-problems-and-root-cause.md` |
| Davis CoPilot / Dynatrace Assist — natural-language-to-DQL, explain-query (DQL2NL), problem summarization, limitations, data privacy — where prompts go (enterprise-vendor LLMs, EU/US processing regions), what Dynatrace may retain, agentic Assist reading live data, PII masking vs blocking | `-[AIOPS]-04-davis-copilot-dynatrace-assist.md` |
...
| Connecting AI out — Workflow AI tasks (DQL cost optimization, problem summaries, forecasts), the Dynatrace MCP server for IDEs/agents, AutomationEngine agentic/approval patterns; also AI Observability, monitoring *your own* GenAI application with Dynatrace as distinct from using Dynatrace's AI | `-[AIOPS]-06-ai-integrations-and-agentic-workflows.md` |
