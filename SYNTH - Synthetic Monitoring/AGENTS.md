...
|---|---|
| Which query path to use for synthetic data — execution events (`dt.synthetic.events`, `dt.synthetic.detailed_events`) for per-failure diagnostics, `timeseries` metrics for rollups, entity definitions and their Smartscape equivalents — plus the key event fields | `-[SYNTH]-01-fundamentals.md` |
| Core Web Vitals from a synthetic browser monitor — why LCP/FCP/CLS are RUM measurements and are *not* exposed as synthetic browser metric keys (go to WEBRUM instead) | `-[SYNTH]-02-browser-monitors.md` |
...
| Defining a private location declaratively — `POST /api/v2/synthetic/locations`, and the API 1.344 `minActiveGateCount`/`maxActiveGateCount`/`nodeSize` properties that finally make one source-controlled definition sufficient | `-[SYNTH]-04-private-locations.md` |
...
| Selecting monitors in bulk — combining AND and OR tag operators in one selection (`tag(a),tag(b)` vs `tag(a,b)`), SaaS 1.344 | `-[SYNTH]-99-best-practice-summary.md` |
