...
- **Primary Grail tags on monitors** (Latest Dynatrace) — set per monitor; every event and metric the monitor produces is then enriched with `primary_tags.*`. The tags come from the **monitor**, not from the host a private-location ActiveGate runs on: *"Tags aren't inherited from the Synthetic location, host group, or any other entity."* Private locations need **ActiveGate 1.331+** for the enrichment (SYNTH-04). Useful for cost-attribution dashboards that want to bill synthetic execution by team or product line.
- **Configuration API → Settings v2 + Platform tokens** for new automation. Synthetic settings are split across per-aspect Settings schemas under `builtin:synthetic.*` — for example `builtin:synthetic.http.outage-handling`, `builtin:synthetic.browser.scheduling`, `builtin:synthetic.primary-grail-tags` and `builtin:synthetic.security-context`. There is no single `builtin:synthetic_test` schema; monitor definitions themselves go through the Synthetic API. Note the v1.88.0 Terraform provider boundary still requires API tokens for synthetic monitors specifically (see AUTOM-04 § Authentication Boundary).

> <sub>**Sources:** [Primary Grail tags for Synthetic (DT docs)](https://docs.dynatrace.com/docs/observe/digital-experience/synthetic/primary-grail-tags-synthetic) — *"After that, all events and metrics generated by the monitor are enriched with these primary Grail tags."* and *"If you're using private Synthetic locations, install ActiveGate version 1.331+. Earlier versions don't enrich the metrics and events produced by Synthetic monitor executions with the monitor's primary Grail tags."*; [Settings API schemas (DT docs)](https://docs.dynatrace.com/docs/dynatrace-api/environment-api/settings/schemas) — 22 `builtin:synthetic.*` schema IDs listed and none named `builtin:synthetic_test`, read 09/18/2026.</sub>
...
- Response validation (status code, text/regex, SSL certificate expiry)
...
- **Browser**: every 5, 10, 15 or 30 minutes; every 1, 2 or 4 hours; or on demand only
- **HTTP**: the same options, plus a 1-minute frequency (1-minute HTTP monitors were observed on the validation tenant, 09/18/2026)

> <sub>**Sources:** [Create and configure a browser monitor (DT docs)](https://docs.dynatrace.com/docs/observe/digital-experience/synthetic/synthetic-app/create-configure-browser-monitors) — *"You can choose a frequency of every 5 , 10 , 15 , or 30 minutes; or 1 , 2 , or 4 hours. You can also set up your monitor to be executed On demand only ."*</sub>
...
| `dt.synthetic.detailed_events` | Request-level detail for HTTP steps (`event.type == "http_step_execution_details"`) |
...
| `browser_monitor_step_execution` | Browser | One per clickpath step |
...
**Entity tables** (`fetch dt.entity.synthetic_test`, `fetch dt.entity.http_check`, `fetch dt.entity.synthetic_location`) hold monitor and location *definitions* — use them to enumerate configured monitors and resolve names. They still exist and still return rows, but DQL queries against them are now documented as deprecated: *"Queries against classic synthetic entities in DQL are deprecated. They remain supported for as long as Dynatrace Classic is supported, so you can adopt the new model at your own pace."*
...
// FALLBACK (classic surface -- deprecated in DQL, and name-only: cloudPlatform, city
