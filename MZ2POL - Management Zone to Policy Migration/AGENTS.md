...
| Replacing MZ-based **filtering** with segments: the eight documented migration scenarios (host group, Kubernetes, host tags, the `Segment`-tag fallback, cloud-native), conversion blockers (no exclusions, derived-data tag propagation, entity operator limits, Davis problem includes), MZ-rule-to-filter mapping, DQL validation, classic-app coexistence | `-[MZ2POL]-05-segments-implementation.md` |
