...
- [FAQ-21: How Do I Get the Right Alerts to the Right People?](FAQ%20-%20Frequently%20Asked%20Questions/markdown/-%5BFAQ%5D-21-alert-notification-routing.md) — Decision layer over ALERT-03 / WFLOW-04: separates the four axes an alerting design conflates — **fires** (anomaly detector, scoped by segment), **routes** (problem-triggered workflow trigger), **sees** (the IAM access model), **filters** (segments, UI only). Built strictly on the documented trigger surface — problem state, event category, severity, affected-entity tags, Delay, Updates, and the additional custom filter query — and the documented recommendation to filter on *"Primary Grail fields, Security context, Custom attributes."* There is no built-in team or routing field: team ownership is a custom attribute you define and set, matched with a DQL matcher (`matchesPhrase` / `matchesValue` / `isNotNull` / `isNull`, boolean and numeric operators, `iAny`). Covers denoise-before-routing via Davis correlation, the **Delay** option for duration suppression (5 min–1 week, evaluated on `dt.duration_marker`) together with the live documentation conflict against the upgrade guide, and three anti-patterns: segments-scope-notification, routing-replaces-visibility, route-before-enrich
