...
- [AIOPS-02: Anomaly Detection](AIOPS%20-%20Dynatrace%20Intelligence/markdown/-%5BAIOPS%5D-02-anomaly-detection.md) — Static thresholds, auto-adaptive, seasonal baselines, and multi-dimensional detection, including how much history a baseline needs and a pre-publish checklist for custom detectors
- [AIOPS-03: Davis AI — Problems and Root Cause Analysis](AIOPS%20-%20Dynatrace%20Intelligence/markdown/-%5BAIOPS%5D-03-davis-ai-problems-and-root-cause.md) — Causal AI engine, problem grouping via Smartscape, DQL for querying problems, and when an empty root cause is expected vs. worth auditing
...
- [FAQ-18: How Do I Monitor Adobe Experience Manager as a Cloud Service?](FAQ%20-%20Frequently%20Asked%20Questions/markdown/-%5BFAQ%5D-18-monitoring-aem-cloud-service.md) — OneAgent full-stack monitoring on Adobe-run containers: requesting enablement through Adobe, what the ticket needs, where the control boundary sits, per-environment licensing, and why enabling it is a cutover rather than a parallel run
- [FAQ-19: How Do I Bring a Third-Party SaaS Platform's Telemetry Into Dynatrace?](FAQ%20-%20Frequently%20Asked%20Questions/markdown/-%5BFAQ%5D-19-third-party-saas-telemetry-integration.md) — The reusable pattern: mapping questions to signal classes, letting the vendor's transport pick the ingestion route, extract-then-discard and the stage-order trap that defeats it, `CUSTOM_*` Smartscape topology from OpenPipeline, and bucket isolation
- [FAQ-20: How Do I Monitor Zscaler With Dynatrace?](FAQ%20-%20Frequently%20Asked%20Questions/markdown/-%5BFAQ%5D-20-monitoring-zscaler.md) — Why ZIA and ZPA logs cannot replace ZDX, the transport split across the three product lines, direct-vs-multi-home ingestion, what to extract and what to discard, a five-node topology model, field mapping, and when synthetics add anything
- [FAQ-21: How Do I Get the Right Alerts to the Right People?](FAQ%20-%20Frequently%20Asked%20Questions/markdown/-%5BFAQ%5D-21-alert-notification-routing.md) — Decision layer over ALERT-03 / WFLOW-04: separates the four axes an alerting design conflates — **fires** (anomaly detector, scoped by segment), **routes** (problem-triggered workflow trigger), **sees** (the IAM access model), **filters** (segments, UI only). Built strictly on the documented trigger surface — problem state, event category, severity, affected-entity tags, Minimum duration, Updates, and the additional custom filter query — and the documented recommendation to filter on *"Primary Grail fields, Security context, Custom attributes."* **Ownership is the built-in answer** — `owner` and `dt.owner` are default tag keys in every environment, so the affected-entity tag filter routes on ownership with no custom field; a custom attribute matched with a DQL matcher is the fallback where ownership cannot be populated (`matchesPhrase` / `matchesValue` / `isNotNull` / `isNull`, boolean and numeric operators, `iAny`). Covers denoise-before-routing via Davis correlation, the **Minimum duration** option for duration suppression (5 min–1 week, evaluated on `dt.duration_marker`) together with the live documentation conflict against the upgrade guide, and three anti-patterns: segments-scope-notification, routing-replaces-visibility, route-before-enrich
- [FAQ-22: What Happened to My PurePath Timings in Grail?](FAQ%20-%20Frequently%20Asked%20Questions/markdown/-%5BFAQ%5D-22-purepath-timings-in-grail.md) — CPU time survived as `span.timing.cpu` / `span.timing.cpu_self`; IO, disk IO, lock, and suspension time were never modeled. What to use instead, why an external call is a client span with no endpoint and no service metrics, the `request_attribute` vs `captured_attribute` array trap, and the dictionary test that settles any "does this field exist" question
- [FAQ-23: How Do I Monitor Box With Dynatrace?](FAQ%20-%20Frequently%20Asked%20Questions/markdown/-%5BFAQ%5D-23-monitoring-box.md) — Why Box webhooks cannot carry an audit feed and the enterprise stream is pull-only, the two-week streaming vs one-year historical split, a worked sizing example at 1,000,000 events/day showing neither endpoint is the bottleneck, complete Workflow and Python collector implementations with the 120-second ceiling as the decision boundary, cursor rules that prevent permanent data loss, and the collector-health check that reports "OK" when nothing has ever arrived
- [FAQ-24: Can We Trust Dynatrace With Our Data?](FAQ%20-%20Frequently%20Asked%20Questions/markdown/-%5BFAQ%5D-24-can-we-trust-dynatrace-with-our-data.md) — The four questions behind "is our data safe?" — captured, stored, accessed, proven — and where each is documented; masking at capture vs ingest, residency and retention, the security-context array trap, and why segments are not access control.
- [FAQ-25: What Actually Carries Over When We Migrate to a New Tenant?](FAQ%20-%20Frequently%20Asked%20Questions/markdown/-%5BFAQ%5D-25-what-carries-over-to-a-new-tenant.md) — Three classes decide every "does X survive the move?" question: configuration moves and is rewritten, identity is re-minted, accumulated state is gone. Each fails differently — loudly, silently, on a delay. Includes the `id == id_classic` comparison that is always false because the fields are different types, the classic → Smartscape mapping read from the semantic dictionary, a per-host baseline-clock query, and why two OneAgents on one host is unsupported rather than merely expensive
...
- [MZ2POL-05: Migrating Management Zone Filtering to Segments](MZ2POL%20-%20Management%20Zone%20to%20Policy%20Migration/markdown/-%5BMZ2POL%5D-05-segments-implementation.md) — Migration scenarios, conversion blockers, and MZ-rule-to-filter mapping
